...
const fetchData = async () => {
  ...
  const fpyRate = data.map((d) => d.fpy_rate); // new FPY line
  const defectRate = data.map((d) => d.defect_rate); // new FPY line
  ...
  chart.setOption({
    ...
    tooltip: {
      trigger: "axis",
      formatter: (params) => {
        const [bar, yieldLine, fpyLine] = params;
